...
public:
    AES256() = default;
    std::string encrypt(std::string plaintext, byte_t *key);
    std::string decrypt(std::string ciphertext, byte_t *key);

private:
    word_t rotWord(word_t x);
    word_t subInt(word_t y);
    word_t subWord(word_t x);
    void keyExpansion(byte_t *key);

    void addRoundKey(int round);
    void shiftRows();
    void subBytes();
    void mixColumns();
    void cipher(byte_t *in, byte_t *out);
    std::string encryptBlock(std::string block, byte_t *key);

    void invShiftRows();
    void invSubBytes();
    byte_t GF256(byte_t x, byte_t y);
    void invMixColumns();
    void invCipher(byte_t *in, byte_t *out);
    std::string decryptBlock(std::string block, byte_t *key);
    ...
    static const uint8_t Nk = 8;
    static const uint8_t Nr = 14;
